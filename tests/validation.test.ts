...
import { validateSolanaAddress, decodeBase58, MAX_INPUT_LENGTH } from "@/lib/solana/address";
...
describe("Solana address validation", () => {
  it("accepts a valid Solana CA", () => {
    const result = validateSolanaAddress(VALID_MINT);
  ...
  it("rejects invalid Base58", () => {
    ...
    for (const bad of ["0OIl0OIl0OIl0OIl0OIl0OIl0OIl0OIl", "not-an-address-at-all-really-here"]) {
      expect(validateSolanaAddress(bad).ok).toBe(false);
    }
  });
  ...
  it("accepts any well-formed address; whether it is a mint is the agent's job", () => {
    const result = validateSolanaAddress(WALLET_ADDRESS);
    expect(result.ok).toBe(true);
  });

  it("rejects multiple addresses in one submission", () => {
    for (const sep of [" ", ",", ", ", ";"]) {
      const result = validateSolanaAddress(`${VALID_MINT}${sep}${VALID_MINT}`);
      expect(result.ok).toBe(false);
  ...
  it("rejects URLs", () => {
    for (const url of [
      ...
      const result = validateSolanaAddress(url);
      expect(result.ok).toBe(false);
    }
  });

  it("rejects other chains with a specific code", () => {
    const evm = validateSolanaAddress("0x" + "a".repeat(40));
    expect(evm.ok).toBe(false);
    if (!evm.ok) expect(evm.code).toBe("unsupported_chain");
  });

  it(`enforces the ${MAX_INPUT_LENGTH}-character maximum`, () => {
    const result = validateSolanaAddress("a".repeat(MAX_INPUT_LENGTH + 1));
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.code).toBe("too_long");
  });

  it("rejects empty input", () => {
    expect(validateSolanaAddress("").ok).toBe(false);
    expect(validateSolanaAddress("   ").ok).toBe(false);
  });
